...
- 页面重新打开时从浏览器存储恢复；选择任一收藏会走与目标输入相同的解析与输入变更链路（清除旧单次结果、终止进行中的连续复测、使旧分享反馈失效），再聚焦采集区等待复核，但不会改写当前地址。
- 存储内容损坏时逐项丢弃无效记录并就地提示可恢复；写入被拒绝（隐私模式、配额等）时保留当前判读，仅提示失败且不显示收藏成功。
